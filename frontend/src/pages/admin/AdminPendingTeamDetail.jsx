...
export default function AdminPendingTeamDetail() {
  const { id } = useParams();
  ...
  const onModerate = async (decision) => {
    ...
    try {
      ...
      navigate("/admin/teams/pending", {
        replace: true,
        state: {
          flash: {
            type: "success",
            message: decision === "approve" ? "チームを承認しました。" : "チームを利用停止にしました。",
          },
        },
      });
    } catch {
    ...
    <div className="adpdetail-root">
      <header className="adpdetail-header">
        <div className="adpdetail-header-row">
          <Link to="/admin/teams/pending" className="adpdetail-back" aria-label="back">
      ...
      <main className="adpdetail-main">
        ...
        {!loading && !team ? <p className="adpdetail-empty">承認待ちチームはありません。</p> : null}
        ...
        {team ? (
          <>
            <section className="adpdetail-hero">
              ...
              <div className="adpdetail-chip">承認待ち</div>
              <h2>{team.name}</h2>
